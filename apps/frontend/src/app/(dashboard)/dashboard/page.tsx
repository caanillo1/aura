...
function ClientSearchFilter({
  ...
  // Sync display text when external clear happens ("Limpiar filtros")
  useEffect(() => {
    ...
    <div ref={containerRef} className="relative flex flex-col gap-1 min-w-[200px]">
      ...
      <div className="relative flex items-center">
        ...
        <span className="absolute right-2.5 pointer-events-none" style={{ color: 'var(--text-muted)' }}>
          {value
            ? <button className="pointer-events-auto p-0.5 rounded hover:bg-white/10 transition-colors" onClick={clear}>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 rounded-xl overflow-hidden shadow-2xl"
          style={{ background: 'var(--card-bg)', border: '1px solid var(--border-subtle)', zIndex: 100 }}>
            ...
            {filtered.length === 0
              ...
              : filtered.map(c => (
                <button
                  ...
                  onClick={() => select(c.id, c.businessName)}
                >
                  {c.businessName}
                </button>
              ))
...
function KpiCard({ label, value, delta, deltaLabel, icon: Icon, color, delay = 0 }: {
  ...
  const deltaColor = neutral ? '#6b7280' : positive ? '#34d399' : '#f87171';
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-1 text-xs font-medium" style={{ color: deltaColor }}>
          ...
          <span>{positive ? '+' : ''}{delta} {deltaLabel}</span>
        </div>
      </div>
      <div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      ...
      <div className="flex items-center justify-between">
        ...
        {href && (
          <Link href={href} className="flex items-center gap-1 text-xs font-medium transition-colors hover:opacity-80"
            style={{ color: '#60a5fa' }}>
...
function Empty({ msg = 'Sin datos' }) {
  return <p className="text-xs text-center py-4" style={{ color: 'var(--text-muted)' }}>{msg}</p>;
}
  ...
  return (
    <div className="flex gap-3 items-center">
      <ResponsiveContainer width={140} height={140}>
        <PieChart>
          <Pie data={data} dataKey="count" innerRadius={42} outerRadius={65} paddingAngle={2} startAngle={90} endAngle={450}>
            {data.map((d, i) => <Cell key={i} fill={colorMap[d.status] ?? '#6b7280'} />)}
          </Pie>
          <Tooltip
        ...
        {data.map(d => {
          ...
          return (
            <div key={d.status} className="flex items-center gap-2 text-xs">
              <span className="flex-shrink-0 w-2.5 h-2.5 rounded-full" style={{ background: colorMap[d.status] ?? '#6b7280' }} />
              <span className="flex-1 truncate" style={{ color: 'var(--text-secondary)' }}>{labelMap[d.status] ?? d.status}</span>
    ...
    <div className="space-y-2">
      {data.map(d => (
        <div key={d.label} className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold w-8 text-right" style={{ color: 'var(--text-primary)' }}>{d.value}%</span>
        </div>
      ))}
    </div>
  );
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    clientsApi.list({ limit: 200 })
      .then((r: any) => setClients(r.data ?? r))
      .catch(() => {});
    usersApi.listAgents({ limit: 100 })
  ...
  // Polling de respaldo: recargar cada 30 segundos independientemente del WebSocket
  useEffect(() => {
    const interval = setInterval(() => loadRef.current(true), 30000);
    return () => clearInterval(interval);
  }, []);

  // WebSocket: recargar al instante cuando llega dashboard:update
  useEffect(() => {
    ...
    // Notificaciones también pueden activar la recarga (cuando hay actividad en el sistema)
    socket.on('notification:new', (n: { type?: string }) => {
      const dashboardTypes = ['ticket', 'acta', 'proyecto', 'orden_servicio'];
      if (n?.type && dashboardTypes.includes(n.type)) {
        if (refreshTimerRef.current) clearTimeout(refreshTimerRef.current);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-28 rounded-2xl animate-pulse" style={{ background: 'var(--card-bg)' }} />
        ))}
      </div>
      <div className="grid grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-52 rounded-2xl animate-pulse" style={{ background: 'var(--card-bg)' }} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4 max-w-[1400px]">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Dashboard Ejecutivo</h2>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Resumen general · se actualiza en tiempo real</p>
        </div>
      ...
      {/* ── Filtros ── */}
      <div className="flex flex-wrap items-end gap-3 mb-4 p-4 rounded-2xl" style={{ background: 'var(--card-bg)', border: '1px solid var(--border-subtle)' }}>
        <div className="flex items-center gap-2 mr-1">
          <Filter className="w-4 h-4" style={{ color: 'var(--text-muted)' }} />
          <span className="text-xs font-semibold" style={{ color: 'var(--text-muted)' }}>Filtros</span>
        </div>
        {/* Client — buscador con normalización de tildes */}
        <ClientSearchFilter
          clients={clients}
        ...
        {/* Clear */}
        {(filters.clientId || filters.agentId || filters.dateFrom || filters.dateTo) && (
          <button
      ...
      {/* ── Row 2: Client Progress | Project Status | Activity Status ── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <SectionCard title="Avance de Implementación por Cliente" href="/implementacion/ordenes" delay={0.1}>
        ...
        <SectionCard title="Estado de Proyectos" href="/implementacion/proyectos" delay={0.15}>
        ...
        <SectionCard title="Actividades por Estado" delay={0.2}>
      ...
      {/* ── Row 3: Implementer Workload | Actas Pending | Tickets ── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <SectionCard title="Carga de Trabajo por Implementador" delay={0.2}>
                ...
                {implementerWorkload.map((iw, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs w-28 truncate flex-shrink-0" style={{ color: 'var(--text-secondary)' }}>{iw.name}</span>
                      <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: 'var(--surface-2)' }}>
                        <motion.div initial={{ width: 0 }} animate={{ width: `${(iw.count / max) * 100}%` }}
                          transition={{ duration: 0.6, ease: 'easeOut' }}
                          className="h-full rounded-full" style={{ background: '#60a5fa' }} />
                      </div>
                      <span className="text-xs font-semibold" style={{ color: 'var(--text-primary)' }}>{iw.count}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <SectionCard title="Actas Pendientes de Firma" href="/implementacion/proyectos" delay={0.25}>
          {pendingActasSignature.length === 0
            ? <Empty msg="Sin actas pendientes" />
            : (
              <div className="space-y-1">
                <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-x-2 px-1 pb-1"
                  style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  {['Cliente', 'Tipo', 'Firmas', 'Días', ''].map(h => (
                    <span key={h} className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>{h}</span>
                  ))}
                </div>
                {pendingActasSignature.slice(0, 5).map(a => (
                  <div key={a.id} className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-x-2 items-center px-1 py-1.5 rounded-lg hover:bg-white/5 transition-colors">
                    ...
                    <div className="flex gap-1">
                      <button className="p-1 rounded hover:bg-green-500/10" title="WhatsApp">
                        <Send className="w-3 h-3" style={{ color: '#34d399' }} />
                      </button>
                      <button className="p-1 rounded hover:bg-blue-500/10" title="Email">
        ...
        <SectionCard title="Tickets" href="/requerimientos/priorizar" delay={0.3}>
      ...
      {/* ── Row 4: Upcoming Activities | Risk ── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <SectionCard title="Próximas Actividades" delay={0.3}>
                  ...
                  return (
                    <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-xl"
                      ...
                      <div className="flex-shrink-0 w-10 text-center">
                        {day === '—'
                          ? <p className="text-[10px] font-medium leading-tight" style={{ color: 'var(--text-muted)' }}>Sin<br/>fecha</p>
                          : <>
                              <p className="text-lg font-bold leading-none" style={{ color: '#60a5fa' }}>{day}</p>
                              {month && <p className="text-[10px] font-semibold" style={{ color: 'var(--text-muted)' }}>{month}</p>}
                            </>
                        }
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{a.name}</p>
                        <p className="text-[11px] truncate" style={{ color: 'var(--text-muted)' }}>{a.client}</p>
                      </div>
                    </div>
                  );
                })}
        ...
        <SectionCard title="Alertas de Riesgo (Notas Críticas)" delay={0.35}>
          {projectRisks.length === 0
            ? (
              <div className="flex flex-col items-center gap-2 py-6">
                <ShieldAlert className="w-8 h-8" style={{ color: '#34d399', opacity: 0.5 }} />
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Sin notas críticas o de alto riesgo registradas</p>
              </div>
            )
            : (
              <div className="space-y-1">
                <div className="grid grid-cols-[auto_1fr_auto_1fr] gap-x-2 px-1 pb-1"
                  style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                ...
                {projectRisks.map((r, i) => {
                  ...
                  return (
                    <div key={i} className="grid grid-cols-[auto_1fr_auto_1fr] gap-x-2 items-center px-1 py-2 rounded-lg hover:bg-white/5 transition-colors">
                      <span className="text-xs font-mono whitespace-nowrap" style={{ color: 'var(--text-secondary)' }}>{r.osNumber}</span>
                      ...
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        <span className="text-[10px] flex-shrink-0" style={{ color: 'var(--text-muted)', opacity: 0.6 }}>{timeAgo(r.createdAt)}</span>
                      </div>
                    </div>
                  );
                })}

...
export function Sidebar({ collapsed, onToggle, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  // Renders the sidebar content; onClose is defined only in mobile mode
      ...
      <>
                  ...
                  <li key={item.label} className="relative">
                    ...
                    >
                      ...
                      {isCollapsed && !isMobile && (
                        <div
                          className="absolute left-full top-0 ml-3 min-w-[180px] rounded-xl overflow-hidden opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-all z-50"
                          style={{
                        ...
                        >
                          <div className="px-4 py-2.5 border-b"
                            ...
                            <p className="text-xs font-bold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>{item.label}</p>
                          </div>
                          {item.children.filter((c) => !c.permission || can(c.permission)).map((child) => {
                            ...
                            return (
                              <Link key={child.href} href={child.href}>
                                ...
                                >
                                  <ChildIcon className="w-4 h-4 shrink-0" style={{ color: childIsActive ? iconColor(true) : 'var(--text-muted)' }} />
                    ...
                    {/* Sub-items (expanded, non-collapsed) */}
                    <AnimatePresence>
                      {!isCollapsed && isOpen && (
                        <motion.ul
                ...
                <li key={item.href}>
                  <Link href={item.href!} onClick={onClose}>
                    ...
                    >
                      ...
                      {isCollapsed && !isMobile && (
                        <div
                          className="absolute left-full ml-3 px-2.5 py-1.5 rounded-lg text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-xl"
                          style={{
                            background: isLight ? 'rgba(255,255,255,0.92)' : '#1e293b',
        ...
        >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group hover:bg-red-500/10 ${isCollapsed ? 'justify-center' : ''}`}
            style={{ color: 'var(--text-muted)' }}
          >
            <LogOut className="w-5 h-5 shrink-0 group-hover:text-red-400 transition-colors" />
            {!isCollapsed && <span className="text-sm font-medium group-hover:text-red-400 transition-colors">Cerrar sesión</span>}
          </button>
        </div>
      </>
    );
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="hidden md:flex flex-col h-full shrink-0 relative z-10"
        style={sidebarStyle}
      >

...
export const serviceOrdersApi = {
  ...
  aiAnalysis: (id: string): Promise<{ narrative: string; insights: string[]; nextActions: string[] }> =>
    api.post(`/service-orders/${id}/ai-analysis`).then((r) => r.data),
  aiChat: (id: string, message: string, history: { role: 'user' | 'assistant'; content: string }[]): Promise<{ reply: string }> =>
    api.post(`/service-orders/${id}/ai-chat`, { message, history }).then((r) => r.data),
...
export const templatesApi = {
  ...
  updateModuleById: (moduleId: string, data: { name?: string; description?: string; tipo?: string | null }): Promise<TemplateModule> =>
...
export const requerimientosApi = {
  ...
  runScheduleNow: (cfg?: { destinatarios?: string[]; estados?: string[]; asunto?: string; mensaje?: string; filtrarSemanaActual?: boolean }) =>
